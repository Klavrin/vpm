...
describe('apm develop', function () {
  ...
  describe("when the repository hasn't been cloned", () =>
    it('clones the repository to ATOM_REPOS_HOME and links it to ATOM_HOME/dev/packages', function () {
      const Develop = require('../lib/develop')
      spyOn(Develop.prototype, 'getRepositoryUrl').andCallFake(function (
        packageName,
        callback
      ) {
      ...
      spyOn(Develop.prototype, 'installDependencies').andCallFake(function (
        packageDirectory,
        options
      ) {
      ...
      return runs(function () {
        ...
        expect(
          fs.existsSync(path.join(repoPath, 'Syntaxes', 'Makefile.plist'))
        ).toBeTruthy()
