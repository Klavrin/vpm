...
describe('apm config', function () {
  ...
  describe('apm config get', () =>
    it('reads the value from the global config when there is no user config', function () {
      ...
      waitsFor(
        'waiting for config get to complete',
        600000,
        () => callback.callCount === 1
      )

      return runs(() =>
        expect(process.stdout.write.argsForCall[0][0].trim()).toBe(
          path.join(process.env.ATOM_HOME, '.apm')
        )
      )
    }))
  ...
  return describe('apm config set', () =>
    it('sets the value in the user config', function () {
      ...
      waitsFor(
        'waiting for config set to complete',
        600000,
        () => callback.callCount === 1
      )
