...
describe('apm ci', function () {
  ...
  it('installs dependency versions as specified by the lockfile', function () {
    const moduleDirectory = path.join(
      temp.mkdirSync('apm-test-'),
      'test-module-with-lockfile'
    )
    wrench.copyDirSyncRecursive(
      path.join(__dirname, 'fixtures', 'test-module-with-lockfile'),
      moduleDirectory
    )
    ...
    return runs(function () {
      ...
      const pjson0 = CSON.readFileSync(
        path.join('node_modules', 'test-module-with-dependencies', 'package.json')
      )
      expect(pjson0.version).toBe('1.1.0')

      const pjson1 = CSON.readFileSync(
        path.join('node_modules', 'test-module', 'package.json')
      )
