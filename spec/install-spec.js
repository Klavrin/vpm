...
describe('apm install', function() {
  ...
  return describe("when installing an atom package", function() {
    ...
    beforeEach(function() {
      const app = express();
      app.get('/node/v10.20.1/node-v10.20.1.tar.gz', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'node-v10.20.1.tar.gz')));
      app.get('/node/v10.20.1/node-v10.20.1-headers.tar.gz', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'node-v10.20.1-headers.tar.gz')));
      app.get('/node/v10.20.1/node.lib', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'node.lib')));
      app.get('/node/v10.20.1/x64/node.lib', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'node_x64.lib')));
      app.get('/node/v10.20.1/SHASUMS256.txt', (request, response) => response.sendFile(path.join(__dirname, 'fixtures', 'SHASUMS256.txt')));
    ...
    describe('when an invalid URL is specified', () => it('logs an error and exits', function() {
    ...
    describe('when a package name is specified', function() {
      ...
      return describe('when multiple releases are available', function() {
        ...
        return describe("when the package has been renamed", () => it('installs the package with the new name and removes the old package', function() {
    ...
    describe('when no path is specified', () => it('installs all dependent modules', function() {
    ...
    describe("when the packages directory does not exist", () => it("creates the packages directory and any intermediate directories that do not exist", function() {
    ...
    describe("when the package contains symlinks", () => it("copies them correctly from the temp directory", function() {
    ...
    describe('when --check is specified', () => it('compiles a sample native module', function() {
      const callback = jasmine.createSpy('callback');
    ...
    describe('when a deprecated package name is specified', () => it('does not install the package', function() {
      const callback = jasmine.createSpy('callback');
    ...
    describe('::cloneFirstValidGitUrl', () => describe('when cloning a URL fails', function() {
      let install = null;
    ...
    describe('::getPackageDependencies', function() {
      ...
      it('excludes repo-local packages in \'packageDependencies\' which have an equivalent normalized path to that in \'dependencies\'', () => expect(packageDependencies['duplicate-package']).toBe(undefined));

      it('includes repo-local packages in \'packageDependencies\' which have different normalized path to that in \'dependencies\'', () => expect(packageDependencies['different-package']).toBe('file:packages/im-batman'));

      it('includes repo-local packages in \'packageDependencies\' that aren\'t in \'dependencies\'', () => expect(packageDependencies['missing-package']).toBe('file:./packages/missing-package'));

      it('includes versioned packages in \'packageDependencies\'', () => expect(packageDependencies['versioned-package']).toBe('1.2.3'));
