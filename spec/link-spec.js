  ...
  describe('when the all flag is true', () =>
    it('unlinks all packages in $ATOM_HOME/packages and $ATOM_HOME/dev/packages', function () {
      ...
      runs(function () {
        ...
        expect(
          fs.existsSync(path.join(atomHome, 'packages', path.basename(packageToLink2)))
        ).toBeTruthy()
        expect(
          fs.existsSync(path.join(atomHome, 'packages', path.basename(packageToLink3)))
        ).toBeTruthy()
  ...
  describe('when the package name is set after --name', () =>
    it('still links and unlinks normally', function () {
      ...
      runs(function () {
        ...
        expect(fs.realpathSync(path.join(atomHome, 'packages', packageName))).toBe(
