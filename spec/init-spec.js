  ...
  describe('when creating a package', function () {
    ...
    describe('when package syntax is JavaScript', () =>
      it('generates the proper file structure', function () {
        ...
        return runs(function () {
          ...
          expect(fs.existsSync(path.join(packagePath, 'keymaps'))).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'keymaps', 'fake-package.json'))
          ).toBeTruthy()
          expect(fs.existsSync(path.join(packagePath, 'lib'))).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'lib', 'fake-package-view.js'))
          ).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'lib', 'fake-package.js'))
          ).toBeTruthy()
          expect(fs.existsSync(path.join(packagePath, 'menus'))).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'menus', 'fake-package.json'))
          ).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'spec', 'fake-package-view-spec.js'))
          ).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'spec', 'fake-package-spec.js'))
          ).toBeTruthy()
          expect(
            fs.existsSync(path.join(packagePath, 'styles', 'fake-package.less'))
          ).toBeTruthy()
    ...
    describe('when package syntax is unsupported', () =>
      it('logs an error', function () {
        const callback = jasmine.createSpy('callback')
        apm.run(
          ['init', '--syntax', 'something-unsupported', '--package', 'fake-package'],
          callback
        )
    ...
    return describe('when converting a TextMate bundle', function () {
      beforeEach(function () {
        ...
        apm.run(
          ['init', '--package', 'fake-package', '--convert', textMateBundlePath],
          callback
        )
      ...
      it('generates the proper file structure', function () {
        expect(fs.existsSync(packagePath)).toBeTruthy()
        expect(
          fs.isFileSync(path.join(packagePath, 'settings', 'fake-package.cson'))
        ).toBe(true)
        expect(
          fs.isFileSync(path.join(packagePath, 'snippets', 'fake-package.cson'))
        ).toBe(true)
        ...
        expect(
          CSON.readFileSync(path.join(packagePath, 'snippets', 'fake-package.cson'))[
            '.source.rd.tm'
          ]['Attach']
        ).toEqual({
        ...
        return expect(
          CSON.readFileSync(path.join(packagePath, 'settings', 'fake-package.cson'))[
            '.source.r'
          ]['editor']
        ).toEqual({
          foldEndPattern: '(^\\s*\\)|^\\s*\\})',
  ...
  describe('when creating a theme', function () {
    it('generates the proper file structure', function () {
      ...
      return runs(function () {
        ...
        expect(
          fs.existsSync(path.join(themePath, 'styles', 'syntax-variables.less'))
        ).toBeTruthy()
        ...
        expect(
          JSON.parse(fs.readFileSync(path.join(themePath, 'package.json'))).name
        ).toBe('fake-theme')
        return expect(
          JSON.parse(fs.readFileSync(path.join(themePath, 'package.json'))).repository
        ).toBe('https://github.com/somebody/fake-theme')
      })
    })
    ...
    return describe('when converting a TextMate theme', function () {
      it('generates the proper file structure', function () {
        ...
        apm.run(
          ['init', '--theme', 'fake-theme', '--convert', textMateThemePath],
          callback
        )
        ...
        return runs(function () {
          ...
          expect(
            fs.readFileSync(
              path.join(themePath, 'styles', 'syntax-variables.less'),
              'utf8'
            )
          ).toContain(`\
...
`)
          expect(
            fs.readFileSync(path.join(themePath, 'styles', 'base.less'), 'utf8')
          ).toContain(`\
  ...
  return describe('when creating a language', function () {
    it('generates the proper file structure', function () {
      ...
      return runs(function () {
        ...
        expect(
          fs.existsSync(path.join(languagePath, 'grammars', 'fake.cson'))
        ).toBeTruthy()
        expect(
          fs.existsSync(path.join(languagePath, 'settings', 'language-fake.cson'))
        ).toBeTruthy()
        expect(
          fs.existsSync(path.join(languagePath, 'snippets', 'language-fake.cson'))
        ).toBeTruthy()
        expect(
          fs.existsSync(path.join(languagePath, 'spec', 'language-fake-spec.coffee'))
        ).toBeTruthy()
        expect(fs.existsSync(path.join(languagePath, 'package.json'))).toBeTruthy()
        expect(
          JSON.parse(fs.readFileSync(path.join(languagePath, 'package.json'))).name
        ).toBe('language-fake')
        return expect(
          JSON.parse(fs.readFileSync(path.join(languagePath, 'package.json'))).repository
        ).toBe('https://github.com/somebody/language-fake')
      })
    })
