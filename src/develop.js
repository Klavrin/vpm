...
`)
    ...
    run(options) {
      ...
      let packageDirectory =
        (left = options.commandArgs.shift()) != null
          ? left
          : path.join(config.getReposDirectory(), packageName)
      ...
      if (fs.existsSync(packageDirectory)) {
        ...
        return this.getRepositoryUrl(packageName, (error, repoUrl) => {
          if (error != null) {
            ...
            const tasks = []
            tasks.push((callback) =>
              this.cloneRepository(repoUrl, packageDirectory, options, callback)
            )

            tasks.push((callback) =>
              this.installDependencies(packageDirectory, options, callback)
            )

            tasks.push((callback) =>
              this.linkPackage(packageDirectory, options, callback)
            )
