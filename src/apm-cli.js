...
for (let commandClass of Array.from(commandClasses)) {
  for (name of Array.from(
    commandClass.commandNames != null ? commandClass.commandNames : []
  )) {
...
`)
  ...
  options
    .boolean('color')
    .default('color', true)
    .describe('color', 'Enable colored output')
...
`
          ...
          if (config.isWin32()) {
            let left2
            const visualStudioVersion =
              (left2 = config.getInstalledVisualStudioFlag()) != null ? left2 : ''
...
var getAtomVersion = (callback) =>
  config.getResourcePath(function (resourcePath) {
    ...
    try {
      let left
      const { version } =
        (left = require(path.join(resourcePath, 'package.json'))) != null
          ? left
          : unknownVersion
  ...
  run(args, callback) {
    ...
    if (args.version) {
      ...
      if ((Command = commands[options.command])) {
        showHelp(
          __guardMethod__(new Command(), 'parseOptions', (o) =>
            o.parseOptions(options.command)
          )
        )
      } else {
        showHelp(options)
      ...
      if (command === 'help') {
        if ((Command = commands[options.commandArgs])) {
          showHelp(
            __guardMethod__(new Command(), 'parseOptions', (o1) =>
              o1.parseOptions(options.commandArgs)
            )
          )
        } else {
          showHelp(options)
