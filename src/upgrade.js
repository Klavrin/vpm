...
`)
      ...
      options
        .alias('l', 'list')
        .boolean('list')
        .describe('list', "List but don't install the outdated packages")
      options.boolean('json').describe('json', 'Output outdated packages as a JSON array')
      options
        .string('compatible')
        .describe(
          'compatible',
          'Only list packages/themes compatible with this Atom version'
        )
      return options
        .boolean('verbose')
        .default('verbose', false)
        .describe('verbose', 'Show verbose debug information')
    }
    ...
    getIntalledPackage(name) {
      ...
      try {
        ...
        if (
          (metadata != null ? metadata.name : undefined) &&
          (metadata != null ? metadata.version : undefined)
        ) {
          return metadata
        }
      } catch (error) {}
      ...
      return config.getSetting('git', (command) => {
        ...
        return this.spawn(
          ...
          function (code, stderr, stdout) {
            ...
            const sha = repo.getReferenceTarget(
              repo.getUpstreamBranch('refs/heads/master')
            )
    ...
    getAvailableUpdates(packages, callback) {
      ...
      return async.mapLimit(
        ...
        function (error, updates) {
          ...
          updates = _.filter(
            updates,
            (update) => update.latestVersion != null || update.sha != null
          )
    ...
    promptForConfirmation(callback) {
      return read(
        { prompt: 'Would you like to install these updates? (yes)', edit: true },
    ...
    upgradePackages(options, callback) {
      ...
      return this.getAvailableUpdates(packages, (error, updates) => {
        ...
        if (options.argv.json) {
          const packagesWithLatestVersionOrSha = updates.map(function ({
            pack,
            latestVersion,
            sha
          }) {
          ...
          tree(updates, function ({ pack, latestVersion, sha }) {
            ...
            latestVersion =
              (latestVersion != null ? latestVersion.green : undefined) ||
              __guard__(
                apmInstallSource != null ? apmInstallSource.sha : undefined,
                (x) => x.green
              )
