...
module.exports = PackageConverter = class PackageConverter {
  ...
  copyDirectories(sourcePath, callback) {
    ...
    try {
      packageName = __guard__(
        JSON.parse(fs.readFileSync(path.join(sourcePath, 'package.json'))),
        (x) => x.packageName
      )
    } catch (error) {}
    if (packageName == null) {
    ...
    return (() => {
      ...
      for (let child of Array.from(fs.readdirSync(directoryPath))) {
        ...
        while (
          ...
          fs.existsSync(convertedPath.toLowerCase())
        ) {
          const extension = path.extname(convertedFileName)
          convertedFileName = `${path.basename(
            convertedFileName,
            extension
          )}-${suffix}${extension}`
  ...
  convertSnippets(packageName, source) {
    ...
    for (let child of Array.from(fs.readdirSync(sourceSnippets))) {
      var left, selector
      const snippet =
        (left = this.readFileSync(path.join(sourceSnippets, child))) != null ? left : {}
  ...
  convertPreferences(packageName, source) {
    ...
    for (let child of Array.from(fs.readdirSync(sourcePreferences))) {
      ...
      const { scope, settings } =
        (left = this.readFileSync(path.join(sourcePreferences, child))) != null
          ? left
          : {}
      if (!scope || !settings) {
        continue
      ...
      if ((properties = this.convertSettings(settings))) {
        ...
        for (let key in properties) {
          ...
          if (preferencesBySelector[selector][key] != null) {
            preferencesBySelector[selector][key] = _.extend(
              value,
              preferencesBySelector[selector][key]
            )
          } else {
    ...
    this.writeFileSync(
      path.join(destination, `${packageName}.cson`),
