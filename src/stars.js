...
`)
      ...
      options
        .alias('i', 'install')
        .boolean('install')
        .describe('install', 'Install the starred packages')
      options
        .alias('t', 'themes')
        .boolean('themes')
      ...
      options
        .alias('u', 'user')
        .string('user')
        .describe('user', 'GitHub username to show starred packages for')
    ...
    requestStarredPackages(requestSettings, callback) {
      return request.get(requestSettings, function (error, response, body) {
        ...
        if (error != null) {
          ...
          let packages = body.filter(
            (pack) =>
              __guard__(pack != null ? pack.releases : undefined, (x) => x.latest) != null
          )
    ...
    logPackagesAsText(user, packagesAreThemes, packages, callback) {
      ...
      tree(
        packages,
        function ({ name, version, description, downloads, stargazers_count }) {
          label = name.yellow
          ...
          if (downloads >= 0 && stargazers_count >= 0) {
            label += ` (${_.pluralize(downloads, 'download')}, ${_.pluralize(
              stargazers_count,
              'star'
            )})`.grey
          }
    ...
    run(options) {
      ...
      const user =
        options.argv.user != null ? options.argv.user.toString().trim() : undefined
