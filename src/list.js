  ...
  List = class List extends Command {
    ...
    constructor() {
      ...
      if ((configPath = CSON.resolve(path.join(config.getAtomDirectory(), 'config')))) {
        try {
          this.disabledPackages = __guard__(
            __guard__(
              __guard__(CSON.readFileSync(configPath), (x2) => x2['*']),
              (x1) => x1.core
            ),
            (x) => x.disabledPackages
          )
        } catch (error) {}
      }
...
`)
      options
        .alias('b', 'bare')
        .boolean('bare')
        .describe('bare', 'Print packages one per line with no formatting')
      options
        .alias('e', 'enabled')
        .boolean('enabled')
        .describe('enabled', 'Print only enabled packages')
      options
        .alias('d', 'dev')
        .boolean('dev')
        .default('dev', true)
        .describe('dev', 'Include dev packages')
      options.boolean('disabled').describe('disabled', 'Print only disabled packages')
      options.alias('h', 'help').describe('help', 'Print this usage message')
      options
        .alias('i', 'installed')
        .boolean('installed')
        .describe('installed', 'Only list installed packages/themes')
      options
        .alias('j', 'json')
        .boolean('json')
        .describe('json', 'Output all packages as a JSON object')
      options
        .alias('l', 'links')
        .boolean('links')
        .default('links', true)
        .describe('links', 'Include linked packages')
      options
        .alias('t', 'themes')
        .boolean('themes')
        .describe('themes', 'Only list themes')
      options
        .alias('p', 'packages')
        .boolean('packages')
        .describe('packages', 'Only list packages')
      return options
        .alias('v', 'versions')
        .boolean('versions')
    ...
    logPackages(packages, options) {
      if (options.argv.bare) {
        ...
        tree(packages, (pack) => {
          ...
          if (
            (pack.apmInstallSource != null ? pack.apmInstallSource.type : undefined) ===
            'git'
          ) {
    ...
    isPackageVisible(options, manifest) {
      return (
        ...
        this.checkExclusiveOptions(
          options,
          'disabled',
          'enabled',
          this.isPackageDisabled(manifest.name)
        )
      )
    }
    ...
    listUserPackages(options, callback) {
      const userPackages = this.listPackages(this.userPackagesDirectory, options).filter(
        (pack) => !pack.apmInstallSource
      )
      if (!options.argv.bare && !options.argv.json) {
        console.log(
          `Community Packages (${userPackages.length})`.cyan,
          `${this.userPackagesDirectory}`
        )
      }
    ...
    listDevPackages(options, callback) {
      ...
      if (devPackages.length > 0) {
        if (!options.argv.bare && !options.argv.json) {
          console.log(
            `Dev Packages (${devPackages.length})`.cyan,
            `${this.devPackagesDirectory}`
          )
        }
      }
    ...
    listGitPackages(options, callback) {
      ...
      if (gitPackages.length > 0) {
        if (!options.argv.bare && !options.argv.json) {
          console.log(
            `Git Packages (${gitPackages.length})`.cyan,
            `${this.userPackagesDirectory}`
          )
        }
      }
