...
`)
      ...
      options
        .alias('d', 'dev')
        .boolean('dev')
        .describe('dev', 'Uninstall from ~/.atom/dev/packages')
      return options
        .boolean('hard')
        .describe('hard', 'Uninstall from ~/.atom/packages and ~/.atom/dev/packages')
    }

    getPackageVersion(packageDirectory) {
      try {
        return __guard__(
          CSON.readFileSync(path.join(packageDirectory, 'package.json')),
          (x) => x.version
        )
      } catch (error) {
        return null
    ...
    run(options) {
      ...
      return async.eachSeries(
        ...
        this.registerUninstall.bind(this),
