...
module.exports = ThemeConverter = class ThemeConverter {
    ...
    if (protocol === 'http:' || protocol === 'https:') {
      ...
      return request.get(requestOptions, (error, response, body) => {
        if (error != null) {
          ...
          return callback(
            `Request to ${this.sourcePath} failed (${response.headers.status})`
          )
        } else {
  ...
  convert(callback) {
    return this.readTheme((error, themeContents) => {
      ...
      fs.writeFileSync(
        path.join(this.destinationPath, 'styles', 'base.less'),
        theme.getStylesheet()
      )
      fs.writeFileSync(
        path.join(this.destinationPath, 'styles', 'syntax-variables.less'),
        theme.getSyntaxVariables()
      )
      return callback()
    })
  }
