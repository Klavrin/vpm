...
const configureRequest = (requestOptions, callback) =>
  loadNpm(function () {
    ...
    const userAgent =
      (left = npm.config.get('user-agent')) != null
        ? left
        : `AtomApm/${require('../package.json').version}`
...
module.exports = {
    ...
    return configureRequest(requestOptions, function () {
      ...
      var tryRequest = function () {
        ...
        return request.get(requestOptions, function (error, response, body) {
          if (
            retryCount > 0 &&
            ['ETIMEDOUT', 'ECONNRESET'].includes(error != null ? error.code : undefined)
          ) {
            retryCount--
            return tryRequest()
          } else {
  ...
  getErrorMessage(response, body) {
    if ((response != null ? response.statusCode : undefined) === 503) {
      ...
      let left
      return (left =
        (body != null ? body.message : undefined) != null
          ? body != null
            ? body.message
            : undefined
          : body != null
